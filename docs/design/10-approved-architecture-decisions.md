...
| AAD-09 | Use native, privacy-conscious aggregate measurement only. | Use separate Cloudflare Web Analytics properties for public and partner surfaces, Cloudflare Stream Analytics for playback, and a minimal Analytics Engine dataset for approved custom action events. Do not add third-party trackers, session replay, cross-surface identity joins, or research-data joins. |
