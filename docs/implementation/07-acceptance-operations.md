...
- GitHub CI now runs the complete locked test suite, including content, vetted-portal boundary, smoke self-test, launch-ledger audit, build, shell, and static accessibility checks.
...
Twelve of the 13 MVP success criteria remain blocked for final launch evidence; the vetted-only public partner boundary now passes. The principal missing inputs are approved public and partner content/assets, two released Stream episodes and captions, the separate private partner repository and Access matrix, final legal/privacy/contact values, external account/DNS/security evidence, manual accessibility/browser review, owner operations rehearsals, and the private legal/IP plan tabletop.
...
- `finalLegalCopyApproved` and `publicDiscoveryEnabled` remain false; the obsolete partner-request feature flag has been removed.
